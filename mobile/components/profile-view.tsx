...
export default function ProfileView({ profile, reload }: { profile: Profile; reload: () => void }) {
  ...
  return <>
    ...
    {profile.is_owner ? <Pressable accessibilityRole="button" style={({ pressed }) => [styles.supportCard, pressed && styles.supportCardPressed]} onPress={() => router.push('/support')}><View style={styles.supportIcon}><Ionicons name="heart-outline" size={23} color={colors.red} /></View><View style={styles.supportBody}><Eyebrow>Free community project</Eyebrow><Text style={styles.supportTitle}>Support Japan47</Text><Text style={styles.supportText}>Optional support helps cover hosting, maintenance, image storage, and future development.</Text></View><Ionicons name="chevron-forward" size={20} color={colors.redDark} /></Pressable> : null}
...
const styles = StyleSheet.create({
  ...
  supportCard: { flexDirection: 'row', alignItems: 'center', gap: spacing.md, padding: spacing.lg, borderWidth: 1, borderColor: '#E8C7C2', borderLeftWidth: 5, borderLeftColor: colors.red, borderRadius: radius.lg, backgroundColor: '#FFF7F5' }, supportCardPressed: { opacity: 0.72 }, supportIcon: { width: 44, height: 44, alignItems: 'center', justifyContent: 'center', borderRadius: 22, backgroundColor: colors.redSoft }, supportBody: { flex: 1 }, supportTitle: { color: colors.ink, fontFamily: typography.title, fontSize: 20, fontWeight: '700' }, supportText: { marginTop: spacing.xs, color: colors.muted, fontSize: 13, lineHeight: 19 },
