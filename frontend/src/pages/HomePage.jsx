...
export default function HomePage() {
  ...
  return <>
    ...
    {loading && <LoadingState />}{error && <ErrorState error={error} />}{data && <>
      ...
      <Feature eyebrow="Community voices" title="Top contributors" text="Travelers expanding Japan 47 for everyone." link="/places" linkLabel="Explore their work" empty={!data.top_contributors.length && 'No contributor profiles are available yet.'}><div className="grid grid--3">{data.top_contributors.map((person, index) => <Link className="contributor-card" to={`/contributors/${person.id}`} key={person.id}><b>#{index + 1}</b>{person.profile_image_url ? <img src={person.profile_image_url} alt="" loading="lazy" decoding="async" width="58" height="58" /> : <span className="avatar">{person.display_name[0]}</span>}<div><h3>{person.display_name}</h3><p>{person.stats.badge.name}</p><strong>{person.stats.points} points</strong></div><img className="badge-mini" src={`/images/badges/${person.stats.badge.filename.replace(/\.png$/i, '.webp')}`} alt="" loading="lazy" decoding="async" width="64" height="64" /></Link>)}</div></Feature>
    </>}
  </>
}
