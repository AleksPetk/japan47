...
export default function PlaceDetailPage() {
  ...
  return <article className="detail page"><SEO title={`${data.name}, ${data.prefecture.name} | Japan47`} description={description} canonicalPath={canonicalPath} robots={data.status === 'published' ? 'index, follow' : 'noindex, nofollow'} type="article" image={socialImage} imageType={imageContentType(socialImage)} structuredData={attractionSchema} /><p className="breadcrumbs"><Link to="/places">Places</Link> / <Link to={`/prefectures/${data.prefecture.name}`}>{data.prefecture.name}</Link> / {data.name}</p>
    ...
    <div className="place-main-image"><button type="button" disabled={!data.image_url} onClick={() => data.image_url && setLightbox({ image_url: data.image_url, caption: data.name })} aria-label={data.image_url ? `Open ${data.name} main image` : `${data.name} has no main image`}><MediaImage src={data.image_url} alt={data.name} priority /></button></div>
    ...
    {gallery.length > 0 && <section className="place-photo-gallery"><header className="section-header"><div><p className="eyebrow">More from this place</p><h2>Gallery</h2></div><span>{gallery.length} of 4 photos</span></header><div className="place-photo-gallery__grid">{gallery.map((image) => <button type="button" key={image.id} onClick={() => setLightbox(image)} aria-label={`Open ${image.caption || data.name} gallery image`}><MediaImage src={image.thumbnail_url || image.image_url} alt={image.caption || `${data.name} gallery photo`} /></button>)}</div></section>}
