...
export default function ProfilePage() {
  ...
  return <article className="profile page"><header className="profile-hero"><div className="profile-identity">{data.profile_image_url ? <img src={data.profile_image_url} alt="" width="130" height="130" /> : <span className="avatar avatar--large">{data.display_name[0]}</span>}<div><p className="eyebrow">Japan 47 contributor</p><h1>{data.display_name}</h1><p>Member since {formatDate(data.joined_at)} · {data.follower_count} followers · {data.following_count} following</p>{data.is_owner && <p>{data.username} · {data.email}</p>}{data.is_owner ? <Link className="button" to="/profile/edit">Edit profile</Link> : user && <button className="button" onClick={toggleFollow}>{data.is_following ? 'Following ✓' : 'Follow'}</button>}</div></div><aside className="badge-card"><img src={`/images/badges/${badge.filename.replace(/\.png$/i, '.webp')}`} alt={badge.name} width="130" height="130" /><div><p>Current badge</p><h2>{badge.name}</h2><strong>{data.stats.points} points</strong><div className="progress"><span style={{ width: `${badge.progress_percent}%` }} /></div><small>{badge.next_name ? `${badge.points_until_next} points to ${badge.next_name}` : 'Highest badge achieved'}</small></div></aside></header><div className="stats"><div><strong>{data.stats.points}</strong><span>Points</span></div><div><strong>{data.stats.published_place_count}</strong><span>Published places</span></div><div><strong>{data.stats.review_count}</strong><span>Reviews</span></div></div>
    ...
    <aside className="profile-support-card"><div><p className="eyebrow">Free community project</p><h2>Support Japan47</h2><p>Japan47 is a free community project. Your support helps cover hosting, maintenance, and future development.</p></div><Link className="button button--primary" to="/support">Learn more</Link></aside>
