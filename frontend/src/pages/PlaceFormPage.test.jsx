...
const fixture = vi.hoisted(() => ({
  ...
  place: {
    ...
    gallery_images: [
      {
        id: 31,
        image_url: 'https://example.test/media/approved-gallery.jpg',
        thumbnail_url: null,
        caption: 'Approved gallery',
      },
    ],
    latest_revision: {
      status: 'pending',
      ...
      gallery_images: [
        {
          id: 41,
          image_url: 'https://example.test/media/proposed-gallery.jpg',
          thumbnail_url: null,
          caption: 'Proposed gallery',
        },
      ],
    },
  },
...
describe('PlaceFormPage revision editing', () => {
  it('loads and updates the pending proposal instead of replacing approved values', async () => {
    render(
      <MemoryRouter initialEntries={['/places/12/edit']}>
        <Routes>
          <Route path="/places/:id/edit" element={<PlaceFormPage />} />
          <Route path="/places/:id/:slug" element={<div>Place detail</div>} />
        </Routes>
      </MemoryRouter>
    )
  ...
  it('shows existing images and submits explicit main and gallery removals', async () => {
    ...
    await userEvent.click(
      screen.getByAltText('Approved gallery').closest('article').querySelector('button')
    )
    await userEvent.click(
      screen.getByAltText('Proposed gallery').closest('article').querySelector('button')
    )
    await userEvent.click(screen.getByRole('button', { name: 'Update pending changes' }))
    ...
    expect(apiMock).toHaveBeenNthCalledWith(3, '/places/12/revision-images/41/', {
      method: 'DELETE',
    })
  })
})
