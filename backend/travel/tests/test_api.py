...
class PublicApiTests(ApiFixture):
    def test_home_cache_rebuilds_media_urls_for_each_request_host(self):
        Place.objects.filter(pk=self.published.pk).update(
            image="place_images/cache-host-test.jpg"
        )
        ...
        self.assertTrue(
            local.json()["latest_places"][0]["image_url"].startswith(
...
class AuthenticationApiTests(ApiFixture):
    ...
    def test_registration_requires_verification_then_login_refresh_me_and_logout(self, send_email):
        ...
        blocked = self.client.post("/api/v1/auth/login/", {"username": "newuser", "password": "StrongPass123!"})
...
class AccountRecoveryApiTests(APITestCase):
    ...
    def test_resend_is_generic_and_obeys_account_cooldown(self, send_message):
        first = self.client.post("/api/v1/auth/resend-verification/", {"email": "PENDING@EXAMPLE.COM"})
        second = self.client.post("/api/v1/auth/resend-verification/", {"email": "pending@example.com"})
        missing = self.client.post("/api/v1/auth/resend-verification/", {"email": "missing@example.com"})
    ...
    def test_resend_endpoint_is_rate_limited_by_ip(self):
        responses = [
            self.client.post("/api/v1/auth/resend-verification/", {"email": f"missing{number}@example.com"})
    ...
    def test_forgot_password_never_reveals_account_existence(self, send_message):
        existing = self.client.post("/api/v1/auth/password-reset/request/", {"email": "pending@example.com"})
        missing = self.client.post("/api/v1/auth/password-reset/request/", {"email": "nobody@example.com"})
    ...
    def test_valid_password_reset_changes_password_and_token_cannot_be_reused(self):
        ...
        self.assertEqual(
            self.client.post("/api/v1/auth/login/", {"username": "pending", "password": "OldStrongPass123!"}).status_code,
            401,
        )
        self.assertEqual(
            self.client.post("/api/v1/auth/login/", {"username": "pending", "password": "NewStrongPass456!"}).status_code,
            200,
        )
    ...
    def test_invalid_and_expired_password_reset_tokens_are_rejected(self):
        uid = urlsafe_base64_encode(force_bytes(self.user.pk))
        token = default_token_generator.make_token(self.user)
        base = {"uid": uid, "new_password": "NewStrongPass456!", "new_password2": "NewStrongPass456!"}
        expired = self.client.post("/api/v1/auth/password-reset/confirm/", {**base, "token": token})
        invalid = self.client.post("/api/v1/auth/password-reset/confirm/", {**base, "token": "invalid-token"})
...
class PlaceMutationApiTests(ApiFixture):
    ...
    def test_published_owner_edit_creates_one_revision_without_changing_live_place(self):
        ...
        self.assertEqual((revision.name, revision.city, revision.status), (
            "Akihabara Updated", "Chiyoda", PlaceRevision.Status.PENDING,
        ))
...
class CommunityApiTests(ApiFixture):
    def test_favorites_and_visited_places(self):
        ...
        self.assertEqual((favorite_delete.headers["Content-Length"], visited_delete.headers["Content-Length"]), ("0", "0"))
        self.assertFalse(Favorite.objects.filter(user=self.author, place=self.published).exists())
        self.assertFalse(VisitedPlace.objects.filter(user=self.author, place=self.published).exists())
...
class SupportTicketApiTests(ApiFixture):
    ...
    def test_support_requires_authentication_and_returns_form_metadata(self):
        self.assertEqual(self.client.get("/api/v1/support/").status_code, status.HTTP_401_UNAUTHORIZED)
        self.assertEqual(self.client.post("/api/v1/support/", {}).status_code, status.HTTP_401_UNAUTHORIZED)
        self.authenticate()
    ...
    def test_creates_sequential_ticket_with_email_snapshot(self):
        self.authenticate()
        first = self.client.post("/api/v1/support/", {
            "category": "account",
        ...
        second = self.client.post("/api/v1/support/", {
            "category": "bug_report",
            "subject": "Map issue",
    ...
    def test_accepts_one_valid_screenshot_and_rejects_fake_images(self):
        self.authenticate()
        response = self.client.post("/api/v1/support/", {
            "category": "bug_report",
    ...
    def test_support_submissions_are_rate_limited(self):
        self.authenticate()
        responses = [
            self.client.post("/api/v1/support/", {
                "category": "other",
    ...
    def test_admin_change_page_is_read_only_for_request_content_and_has_mailto(self):
        self.authenticate()
        created = self.client.post("/api/v1/support/", {
            "category": "account",
        ...
        response = self.client.get(f"/{settings.ADMIN_PATH}travel/supportticket/{ticket.pk}/change/")
        self.assertEqual(response.status_code, 200)
